...
from openpyscad.ops_math import OpsMath

class Custom2dShapes:
	...
	def regular_polygon(num, r, offset=0):
		...
		for i in range(num):
			angle = 90 + ((i * 360) / num)
			points.append([r * OpsMath.cos(angle), r * OpsMath.sin(angle)])
	...
	def sector(r, angle, sectors=360, offset=0):
		...
		angle = angle%360
		height = OpsMath.sin(angle)*r
		width = OpsMath.cos(angle)*r
		
		points = [[0,0], [0,0],[0,0]]
		
		if(angle<=90):
			max_height = OpsMath.sin(angle)*r
			points = [
				[0,0],
				[r, 0],
				[r, max_height],
